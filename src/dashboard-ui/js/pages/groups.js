...
(() => {
  window.GroupsPage = {
    async render() {
      ...
      document.getElementById("btn-load-groups").addEventListener("click", () => this.loadGroups());
      document
    ...
    async loadGroups() {
      ...
      el.innerHTML = '<div class="skeleton skeleton-card w-full" style="height:150px"></div>';
      ...
      if (groups.length === 0) {
        el.innerHTML = '<div class="empty-state"><h3>No Groups</h3></div>';
        return;
      }
    ...
    showCreateModal() {
      ...
      document.getElementById("create-group-form").addEventListener("submit", async (e) => {
        e.preventDefault();
        const result = await API.request(`/groups/${session}/create`, {
          method: "POST",
          body: JSON.stringify({
            groupName: document.getElementById("grp-name").value,
