...
(function () {
	...
	function loadPollingConfig() {
		try {
			...
			return {
				activeMs: Math.min(
					Math.max(
						Number(parsed.activeMs) || DEFAULT_POLL_ACTIVE_MS,
						1500,
					),
					20000,
				),
				idleMs: Math.min(
					Math.max(
						Number(parsed.idleMs) || DEFAULT_POLL_IDLE_MS,
						5000,
					),
					120000,
				),
	...
	function savePollingConfig(activeMs, idleMs) {
		const payload = {
			activeMs: Math.min(
				Math.max(Number(activeMs) || DEFAULT_POLL_ACTIVE_MS, 1500),
				20000,
			),
			idleMs: Math.min(
				Math.max(Number(idleMs) || DEFAULT_POLL_IDLE_MS, 5000),
				120000,
			),
		};
	...
	function extractMessagePayload(msg) {
		...
		if (m.imageMessage?.caption)
			return { text: m.imageMessage.caption, unsupported: false };
		if (m.videoMessage?.caption)
			return { text: m.videoMessage.caption, unsupported: false };
	...
	function extractActionButtons(msg) {
		...
		if (m?.buttonsResponseMessage?.selectedDisplayText) {
			output.push(
				`Selected: ${m.buttonsResponseMessage.selectedDisplayText}`,
			);
		}
	...
	window.ChatRoomsPage = {
        ...
        <div class="chatrooms-page">
          ...
          <div class="chatrooms-layout">
			...
			const pollToggleBtn = document.getElementById(
				"chatrooms-polling-toggle",
			);
			const pollPanelRow = document.getElementById(
				"chatrooms-polling-panel-row",
			);
			const pollPanel = document.getElementById(
				"chatrooms-polling-panel",
			);
			...
			searchInput.addEventListener("input", async () => {
				await this.renderChatListFromCache(
			...
			pollToggleBtn.addEventListener("click", () => {
				const isOpen =
					pollPanelRow && !pollPanelRow.hasAttribute("hidden");
				if (!isOpen) {
					pollPanelRow?.removeAttribute("hidden");
					requestAnimationFrame(() =>
						pollPanel.classList.add("is-open"),
					);
			...
			pollApplyBtn.addEventListener("click", () => {
				const next = savePollingConfig(
					pollActiveInput.value,
					pollIdleInput.value,
				);
			...
			document
				.getElementById("chatrooms-compose-form")
				.addEventListener("submit", async (e) => {
					e.preventDefault();
					const receiverInput = document.getElementById(
						"chatrooms-compose-receiver",
					);
					const textInput = document.getElementById(
						"chatrooms-compose-text",
					);
					...
					if (!sendResult.success) {
						Toast.error(
							sendResult.message || "Failed to send message",
						);
						return;
					}
		...
		handlePresenceEvent(ev) {
			...
			Object.entries(presences).forEach(([participantJid, p]) => {
				...
				if (
					normalizeJid(participantJid) === normalizeJid(activeSelfJid)
				)
					return;
		...
		updateMessageBubbleStatus(messageId, receiptType) {
			...
			bubbles.forEach((bubble) => {
				...
				// Update in DOM
				const timestamp =
					timeEl.textContent.match(/^[^•]+/)?.[0] || "Now";
		...
		async loadChatList(showToastOnError = true) {
			...
			chatCache = (result.data || [])
				.filter((chat) =>
					isGroupMode ? isGroupJid(chat.id) : isOneToOneJid(chat.id),
				)
				.filter(
					(chat) =>
						normalizeJid(chat.id) !== normalizeJid(activeSelfJid),
				)
		...
		async renderChatListFromCache(query = "") {
			...
			const filtered = chatCache.filter((chat) => {
				...
				const haystack =
					`${normalizeContactName(chat)} ${chat.id || ""}`.toLowerCase();
			...
			if (
				!selectedJid ||
				!filtered.some((chat) => chat.id === selectedJid)
			) {
				selectedJid = filtered[0].id;
				await this.loadConversation(selectedJid);
				const first = sidebar.querySelector(
					`[data-jid="${selectedJid}"]`,
				);
		...
		markPendingBubbleSent(pendingId, serverTimestamp) {
			...
			const ts =
				normalizeTimestampSeconds(serverTimestamp) ||
		...
		async loadGroupMembers(jid) {
			...
			if (!panel) return;
			panel.innerHTML =
				'<div class="text-sm text-muted">Loading members...</div>';
			const result = await API.get(
			...
			if (!members.length) {
				panel.innerHTML =
					'<div class="text-sm text-muted">No members metadata available.</div>';
				return;
			}
        ...
        <form class="chatroom-input" id="chatroom-send-form">
          ...
          <button class="btn btn-primary" type="submit">Send</button>
			...
			document
				.getElementById("chatroom-reload-msg")
				.addEventListener("click", async () => {
					await this.loadConversation(jid, false);
				});

			document
				.getElementById("chatroom-mark-read")
				.addEventListener("click", async () => {
			...
			if (isGroupMode) {
				document
					.getElementById("chatroom-load-members")
					.addEventListener("click", async () => {
			...
			if (!result.success) {
				if (showToastOnError)
					Toast.error(
						result.message || "Failed to load conversation",
					);
			...
			document
				.getElementById("chatroom-send-form")
				.addEventListener("submit", async (e) => {
					e.preventDefault();
					const input = document.getElementById("chatroom-text");
					...
					try {
						...
						this.markPendingBubbleSent(
							pendingId,
							sendResult?.data?.messageTimestamp,
						);
                  ...
                  ${
						actionButtons.length > 0
							? `<div class="msg-actions">${actionButtons
									.map(
										(label) =>
											`<span class="msg-action-pill">${escapeHtml(label)}</span>`,
									)
									.join("")}</div>`
							: ""
					}
		...
		async refreshCurrentConversation(showToastOnError = false) {
			...
			if (!result.success) {
				if (showToastOnError) {
					Toast.error(
						result.message || "Failed to refresh conversation",
					);
				}
				return;
			}
