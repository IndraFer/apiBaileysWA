...
x-webhook-secret: &lt;optional secret&gt;</pre>
        ...
        <div class="card">
          ...
          <div id="webhook-logs-container">${this.renderLogs(logs)}</div>
			...
			const payloadRefToggle =
				document.getElementById("payload-ref-toggle");
			const payloadRefDetails = document.getElementById(
				"payload-ref-details",
			);
			const payloadEventSelect = document.getElementById(
				"payload-event-select",
			);
			const payloadEventJson =
				document.getElementById("payload-event-json");

			const renderRealtimePayload = (eventName) => {
				if (!payloadEventJson) return;
				const payload = buildRealtimePayload(
					sampleSessionId,
					eventName || defaultPreviewEvent,
				);
			...
			payloadEventSelect?.addEventListener("change", (event) => {
				renderRealtimePayload(
					event.target?.value || defaultPreviewEvent,
				);
			});
			...
			document
				.querySelectorAll(".webhook-secret-toggle")
				.forEach((btn) => {
		...
		renderSessionWebhook(session) {
			...
			const selectedEvents =
				Array.isArray(session.webhookEvents) &&
				session.webhookEvents.length > 0
		...
		async saveWebhook(e, sessionId) {
			...
			const webhookSecret = document
				.getElementById(`wh-secret-${sessionId}`)
				.value.trim();
