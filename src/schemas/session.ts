...
export const autoReplySchema = z.object({
  ...
  timeStart: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, "Invalid start time format (HH:mm)").optional(),
  timeEnd: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, "Invalid end time format (HH:mm)").optional(),
}).optional();
